  ...
  subscriptions.push(
    emitter.addListener(
      ...
      (event: NativeProgressEvent) => {
        if (event.downloadId !== record.id) return;
        useDownloadsStore
          .getState()
          .updateProgress(
            record.id,
...
export const httpDownloadBackend: DownloadBackend = {
    ...
    const task = File.createDownloadTask(
      ...
      new File(toFileUri(destination.stagingPath)),
      {
        ...
        onProgress: progress => {
          ...
          if (!active || active.cancelled || active.task !== task) return;
  ...
  async pause(downloadId: string): Promise<void> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new DownloadPauseSupportError(`Unable to pause this download. ${message}`);
    }
  },
